...
is 9001.  By default, this reads a request from stdin (to EOF)
...
fn make_config(args: &Args) -> Arc<rustls::ClientConfig> {
    ...
    emitter.dump(&doc["TCP_Listener"][3]).unwrap();

    let ports:Vec<&str> = out_str.split(":").collect();
    let cafile = ports[1].replace("\"", "");
...
fn main() {
    ...
    let addr = lookup_ipv4(args.arg_hostname.as_str(), _n_port);
