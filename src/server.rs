...
impl OpenConnection {
    ...
    fn incoming_plaintext(&mut self, buf: &[u8]) {
        match self.mode {
            ...
            ServerMode::Uds => {
                ...
                let mut _unixstream = match UnixStream::connect(&socket) {
                    Err(_) => panic!("server is not running"),
                    Ok(mut _unixstream) => {
                        if let Err(_e)  = _unixstream.write_all(buf) {
                            panic!("Sending data failed.");
                        }
                    },
                };
                self.send_uds_response_once();
            }
    ...
    fn send_uds_response_once(&mut self) {
        let response =
            b"UDS OK\r\nConnection: close\r\n\r\nHello world from rustls tlsserver\r\n";
        if !self.sent_uds_response {
            self.tls_conn
                .writer()
                .write_all(response)
                .unwrap();
            self.sent_uds_response = true;
            self.tls_conn.send_close_notify();
        }
    }
...
const USAGE: &str = "
Runs a TLS server on :PORT.  The default PORT is 9001.
`echo' mode means the server echoes received data on each connection.
`http' mode means the server blindly sends a HTTP response on each
connection.
`uds' mode means the server blindly sends a UDS Stream response on each
connection.
...
Usage:
  server --certs CERTFILE --key KEYFILE [--suite SUITE ...] \
     [--proto PROTO ...] [--protover PROTOVER ...] [options] echo
  server --certs CERTFILE --key KEYFILE [--suite SUITE ...] \
     [--proto PROTO ...] [--protover PROTOVER ...] [options] http
  server --certs CERTFILE --key KEYFILE [--suite SUITE ...] \
     [--proto PROTO ...] [--protover PROTOVER ...] [options] uds
  server --certs CERTFILE --key KEYFILE [--suite SUITE ...] \
...
Options:
    -p, --port PORT     Listen on PORT [default: 9001].
...
fn lookup_versions(versions: &[String]) -> Vec<&'static rustls::SupportedProtocolVersion> {
    ...
    for vname in versions {
        let version = match vname.as_ref() {
            ...
            _ => panic!(
                "cannot look up version '{}', valid version is '1.3'",
                vname
            ),
        };
        out.push(version);
...
fn make_config(args: &Args) -> Arc<rustls::ServerConfig> {
    ...
    let mut out_str = String::new();
    let mut emitter = YamlEmitter::new(&mut out_str);
    emitter.dump(&doc["TCP_Listener"][2]).unwrap();

    let ports:Vec<&str> = out_str.split(":").collect();
    let certfile = ports[1].replace("\"", "");
    
    out_str = String::new();
    emitter = YamlEmitter::new(&mut out_str);
    emitter.dump(&doc["TCP_Listener"][1]).unwrap();

    let keyname:Vec<&str> = out_str.split(":").collect();
    let keyfile = keyname[1].replace("\"", "");
...
fn main() {
    ...
    let mut out_str = String::new();
    let mut emitter = YamlEmitter::new(&mut out_str);
    emitter.dump(&doc["TCP_Listener"][0]).unwrap();

    let ports:Vec<&str> = out_str.split(":").collect();
    let _port = ports[1].replace("\"", "");
    let mut tcpaddr = "0.0.0.0:".to_string();
    tcpaddr.push_str(&_port);

    let _n_port: u16 = _port
        .trim()
        .parse()
    ...
    let mut addr: net::SocketAddr = tcpaddr.parse().unwrap();
    addr.set_port(args.flag_port.unwrap_or(_n_port));
